...
def batchnorm_backward(dout, cache):
    ...
    N,D=x.shape
    dx_hat=dout*gamma
    dsample_var=dx_hat*(x-sample_mean)*(-0.5)*(sample_var+eps)**(-1.5)
    dsample_var=np.sum(dsample_var,axis=0,keepdims=True)
    dsample_mean=dx_hat*(-1)/np.sqrt(sample_var+eps)-dsample_var*(x-sample_mean)/N
    dsample_mean=np.sum(dsample_mean,axis=0,keepdims=True)
    
    dx1=dx_hat/(np.sqrt(sample_var+eps))
    dx2=dsample_var*2*(x-sample_mean)/N
    dx3=dsample_mean/N
    dx=dx1+dx2+dx3
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    output_wight=1+(W+2*pad-WW)//stride
    out=np.zeros((N,F,output_height,output_wight))
    for i in range(output_height):
        for j in range(output_wight):
            x_pad_mask=x_pad[:,:,stride*i:stride*i+HH,stride*j:stride*j+WW]
            for k in range(F):
...
def max_pool_forward_naive(x, pool_param):
    ...
    output_width=1+(H-pool_height)//stride
